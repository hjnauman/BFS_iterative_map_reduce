 ...
 * WEIGHTS = the list of weights emanating from the node (e.g. 1,3,5,10)
...
public class GraphSearch extends Configured implements Tool {
    ...
    public void map(LongWritable key, Text value, OutputCollector<IntWritable, Text> output,
      ...
      if (node.getColor() == Node.Color.GRAY) {
        for (int v : node.getEdges()) {
          ...
          vnode.setDistance(node.getDistance() + node.getWeights().get(node.getEdges().indexOf(v)));
  ...
  public static class Reduce extends MapReduceBase implements
    ...
    public void reduce(IntWritable key, Iterator<Text> values,
      ...
      while (values.hasNext()) {
        ...
        // Save the minimum distance and color of minimum distance
  ...
  public int run(String[] args) throws Exception {
    ...
    while (keepGoing(iterationCount, numberOfIterations)) {
  ...
  private boolean keepGoing(int iterationCount, int numberOfIterations) {
